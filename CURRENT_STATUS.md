...
| 6 | Research & Enrichment | **6A+6B+6C implemented; 6D Socrata/live-provider foundation + pinned DNS/SSRF destination enforcement + explicit text/number identity typing implemented; one controlled NYC PLUTO live validation completed 2026-08-15; provider remains disabled** | 6A: research package skeleton, Manual/Null providers, registry, candidate selection, append-only `ResearchResult` persistence, CLI. 6B: fail-closed ResearchResult cache, nested reliability config, in-process token-bucket pacing, bounded transient retries, `--no-cache`. 6C: `research_review_items` human review queue, additive migration, atomic pending-only close, no Contact/Lead/Property/compliance mutations. 6D slice: sync `ResearchHttpClient`, endpoint/host policy, `SocrataOpenDataProvider`, registry/config DTOs, deterministic matching, compact provenance, pinned DNS/SSRF enforcement (TCP only to validated public resolved addresses; TLS/SNI/Host remain the configured hostname), config-driven `parcel_value_type` / `account_value_type` (`text` default or `number`). On 2026-08-15, one authorized unauthenticated SODA 2.x `/resource` lookup of `nyc_dcp_pluto` (`64uk-42ks`, BBL `4142600080`) returned HTTP 200 / `success` / `found` / `result_count` 1 / `match_mode` `exact_parcel` with evidence fields `parcel_id`, `owner_name_on_record`, `current_address`. No token, no persistence, no county pointer, no retry, no v3 fallback. The provider was restored to `verified_for_automated_access: false`; `access_reviewed_on` is absent. No county points to it. Default remains `manual_lookup`. This is not production enablement, not commercial/legal clearance, and not authorization of another live request. ArcGIS and generic REST are unimplemented. See `docs/architecture/PHASE6_RESEARCH_DESIGN.md`. |

**Not implemented (do not treat as present):** production-enabled live government endpoints, ArcGIS, generic REST providers, Contact materialization, `scoring/`, `crm/`, `reports/`, `integrations/` (Airtable), `pipeline/` orchestration, `dashboard/`. One controlled NYC PLUTO Socrata lookup was live-validated on 2026-08-15; `nyc_dcp_pluto` remains disabled and is not selected by any county.
...
Remaining Phase 6D work is later increments: ArcGIS, then generic REST. Do not treat the Socrata milestone as production enablement of PLUTO, as completion of all of 6D, or as clearance to begin 6E/6F.
