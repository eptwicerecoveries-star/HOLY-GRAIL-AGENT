...
| 6 | Research & Enrichment | **6A+6B+6C implemented; 6D Socrata/live-provider foundation implemented; controlled live endpoint validation pending** | 6A: research package skeleton, Manual/Null providers, registry, candidate selection, append-only `ResearchResult` persistence, CLI. 6B: fail-closed ResearchResult cache, nested reliability config, in-process token-bucket pacing, bounded transient retries, `--no-cache`. 6C: `research_review_items` human review queue, additive migration, atomic pending-only close, no Contact/Lead/Property/compliance mutations. 6D slice: sync `ResearchHttpClient`, endpoint/host policy, `SocrataOpenDataProvider`, registry/config DTOs, deterministic matching, compact provenance. **No real government/open-data endpoint has been called or verified.** ArcGIS and generic REST are unimplemented. Default provider remains `manual_lookup`. See `docs/architecture/PHASE6_RESEARCH_DESIGN.md`. |
...
5. Lead creation
6. Research & Enrichment — **6A+6B+6C complete; 6D Socrata/live-provider foundation implemented; controlled live endpoint validation pending. ArcGIS and generic REST are not implemented.**
...
**Phase 6D live-endpoint validation (not started):** select and approve one real official dataset and access method. Do not point a county at Socrata until that approval. Hostname checks run on the URL string before connect; they do **not** pin the resolved IP, so DNS rebinding / private resolution after validation remains a pre-live blocker. Resolved-IP enforcement (or equivalent post-resolution private-address protection) must be implemented and reviewed before the first real Socrata provider is enabled. ArcGIS and generic REST remain unimplemented.
