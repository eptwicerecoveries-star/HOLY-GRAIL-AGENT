...
5. Lead creation
6. Research & Enrichment — **6A+6B+6C complete; 6D Socrata/live-provider foundation + pinned DNS/SSRF destination enforcement + explicit text/number identity typing implemented; one controlled NYC PLUTO live validation completed 2026-08-15; `nyc_dcp_pluto` remains disabled. 6D-ArcGIS-A generic offline FeatureServer foundation is complete/committed (not live-validated); `example_arcgis` remains disabled. 6D-ArcGIS-B disabled official Franklin County candidate config is present; not live-validated; ArcGIS-C is not using Franklin because exact PARCELID is unresolved. 6D-ArcGIS-C preparation disabled Lake County candidate config is present; not live-validated. 6D-ArcGIS-C live request is NOT authorized. Phase 6D is still in progress. Generic REST remains after ArcGIS and is not implemented. 6E/6F are not started.**
...
Remaining Phase 6D work is later increments: a separately authorized 6D-ArcGIS-C controlled live ArcGIS request (**NOT authorized** here; not live-validated), then generic REST after ArcGIS. 6D-ArcGIS-A is a complete/committed offline foundation. 6D-ArcGIS-B added disabled official Franklin County Auditor config only; Franklin is not the ArcGIS-C live candidate because exact `PARCELID` is unresolved. 6D-ArcGIS-C preparation added disabled Lake County Property Appraiser config only; no live Lake `/query` occurred. Phase 6D is still **in progress**. Phase 6E and Phase 6F are **not started**. Do not treat Franklin or Lake API access as working. Do not treat 6D-ArcGIS-A/B/C-preparation as live validation, as completion of all of 6D, or as clearance to begin 6E/6F. Do not treat the Socrata milestone as production enablement of PLUTO.
...
Lake County Property Appraiser Tax Parcels is the selected **disabled** ArcGIS-C preparation candidate (`lake_county_fl_pa_tax_parcels`) because official PA/GIS pages supply stronger `AltKey` identity evidence. The provider is not enabled and is not live-validated. Default remains `manual_lookup`. No county points to it. `verified_for_automated_access: false`. `access_reviewed_on` is absent. No token. `ParcelNumber` is not selected.

This section does **not** claim legal permission to automate, commercial-use approval, bulk collection approval, production access, proven anonymous `/query`, or proven token-free operation.
...
- Lake County Connect exposes **AltKey** as an explicit search type

Classification: `2866713` = **EXACT ARCGIS QUERY IDENTIFIER EVIDENCE FOR `AltKey`**. This does **not** prove a future `/query` will succeed.

**NO LOWER-PII TEST RECORD SELECTED.** `2866713` remains a technically verified identifier example for human consideration only and is **not** authorized here as a live-test value.
...
Official layer metadata is publicly listed and was visible locally. Some cloud-origin metadata fetches were WAF-blocked. Local production-network `/query` behavior is **NOT VERIFIED**. If the eventual single request is blocked: **STOP**. No bypass, no alternate host, no browser emulation, no token workaround, no second request.
