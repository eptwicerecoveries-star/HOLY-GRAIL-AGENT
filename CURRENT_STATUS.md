...
5. Lead creation
6. Research & Enrichment — **6A+6B+6C complete; 6D Socrata/live-provider foundation + pinned DNS/SSRF destination enforcement + explicit text/number identity typing implemented; controlled live endpoint validation pending. ArcGIS and generic REST are not implemented.**
...
**Phase 6D live-endpoint validation (not started):** NYC PLUTO (`data.cityofnewyork.us` / `64uk-42ks`) is the leading first-live Socrata candidate only. It has not been technically validated live. The Socrata adapter now supports explicitly configured text or number identity fields (offline-tested). Do not point a county at Socrata until a separate live-validation approval. Pinned DNS/SSRF destination enforcement is implemented for the research HTTP client (TCP connects only to validated public resolved addresses; TLS/SNI/Host stay bound to the original hostname). This does not eliminate every conceivable DNS/network attack. Automated tests are offline. No real Socrata endpoint has been approved or validated. No live provider has been enabled. ArcGIS and generic REST remain unimplemented.
