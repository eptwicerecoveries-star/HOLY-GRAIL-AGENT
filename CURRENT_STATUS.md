...
5. Lead creation
6. Research & Enrichment — **6A+6B+6C complete; 6D Socrata/live-provider foundation + pinned DNS/SSRF destination enforcement implemented; controlled live endpoint validation pending. ArcGIS and generic REST are not implemented.**
...
**Phase 6D live-endpoint validation (not started):** select and approve one real official dataset and access method. Do not point a county at Socrata until that approval. Pinned DNS/SSRF destination enforcement is implemented for the research HTTP client (TCP connects only to validated public resolved addresses; TLS/SNI/Host stay bound to the original hostname). This does not eliminate every conceivable DNS/network attack. Automated tests are offline. No real Socrata endpoint has been approved or validated. Live activation still requires a separate official-dataset and access review. ArcGIS and generic REST remain unimplemented.
