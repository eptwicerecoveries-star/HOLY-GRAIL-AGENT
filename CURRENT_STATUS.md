...
| 6 | Research & Enrichment | **6A+6B+6C implemented; 6D Socrata/live-provider foundation + pinned DNS/SSRF destination enforcement + explicit text/number identity typing implemented; NYC PLUTO configured as an unverified disabled candidate; controlled live endpoint validation pending** | 6A: research package skeleton, Manual/Null providers, registry, candidate selection, append-only `ResearchResult` persistence, CLI. 6B: fail-closed ResearchResult cache, nested reliability config, in-process token-bucket pacing, bounded transient retries, `--no-cache`. 6C: `research_review_items` human review queue, additive migration, atomic pending-only close, no Contact/Lead/Property/compliance mutations. 6D slice: sync `ResearchHttpClient`, endpoint/host policy, `SocrataOpenDataProvider`, registry/config DTOs, deterministic matching, compact provenance, pinned DNS/SSRF enforcement (TCP only to validated public resolved addresses; TLS/SNI/Host remain the configured hostname), config-driven `parcel_value_type` / `account_value_type` (`text` default or `number`). **No real government/open-data endpoint has been called or verified.** `nyc_dcp_pluto` is configured and listable with `verified_for_automated_access: false`; no county points to it. ArcGIS and generic REST are unimplemented. Default provider remains `manual_lookup`. See `docs/architecture/PHASE6_RESEARCH_DESIGN.md`. |

**Not implemented (do not treat as present):** verified live government endpoints, ArcGIS, generic REST providers, Contact materialization, `scoring/`, `crm/`, `reports/`, `integrations/` (Airtable), `pipeline/` orchestration, `dashboard/`.
...
5. Lead creation
6. Research & Enrichment — **6A+6B+6C complete; 6D Socrata/live-provider foundation + pinned DNS/SSRF destination enforcement + explicit text/number identity typing implemented; NYC PLUTO is an unverified disabled candidate; controlled live endpoint validation pending. ArcGIS and generic REST are not implemented.**
...
**Phase 6D live-endpoint validation (not started):** NYC PLUTO (`nyc_dcp_pluto`, `data.cityofnewyork.us` / `64uk-42ks`) is configured as an **unverified disabled** Socrata candidate (`verified_for_automated_access: false`). No county points to it. Typed Number BBL identity support is implemented and offline-tested. No PLUTO live request has occurred. Final human access review and approval of one controlled live request remain pending. Pinned DNS/SSRF destination enforcement is implemented for the research HTTP client (TCP connects only to validated public resolved addresses; TLS/SNI/Host stay bound to the original hostname). This does not eliminate every conceivable DNS/network attack. Automated tests are offline. No live provider has been enabled. ArcGIS and generic REST remain unimplemented.

Do not begin 6E/6F. Do not treat empty compliance YAML as a reason to bypass the fail-closed gate.
